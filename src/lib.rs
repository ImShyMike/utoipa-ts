...
//! async fn list_todos() {}
//!
//! utoipa_ts::export!("types/api.d.ts");
//!
//! fn main() {}
//! ```
//!
...
//! # Export path
//!
//! [`export!`] writes to `types.d.ts` by default. You can pass a path:
//!
//! ```rust
//! utoipa_ts::export!("types/api.d.ts");
//! ```
...
//! indexed by path and method, e.g. `Api["/todos"]["GET"]`.
//!
...
//! - HTTP method
//! - `path = "..."`
//! - `params(...)`, split by query, path, header, and cookie location
//! - `request_body = Type` and `request_body(content = Type, ...)`
//! - response status/body pairs
//! - response `content_type = "..."` and `headers(...)`
//! - parameter, request body, and response descriptions
...
pub const DEFAULT_EXPORT_PATH: &str = "types.d.ts";
const DEFAULT_EXPORT_FILE_NAME: &str = "types.d.ts";
...
impl From<utoipa::openapi::path::ParameterIn> for ParameterLocation {
    fn from(location: utoipa::openapi::path::ParameterIn) -> Self {
        match location {
            utoipa::openapi::path::ParameterIn::Query => Self::Query,
            utoipa::openapi::path::ParameterIn::Header => Self::Header,
            utoipa::openapi::path::ParameterIn::Path => Self::Path,
            utoipa::openapi::path::ParameterIn::Cookie => Self::Cookie,
        }
    }
}
...
impl<'a> EndpointRender<'a> {
    ...
    where
        T: ts_rs::TS + 'static,
    {
        self.param_with_location::<T>(name, ParameterLocation::Path, None);
    }

    pub fn param_with_location<T>(
        &mut self,
        name: &'static str,
        ...
        T: ts_rs::TS + 'static,
    {
        self.spec.params.push(ParameterSpec {
            location,
            field: FieldSpec {
                name: name.to_owned(),
        ...
        self.spec.params.extend(
            T::into_params(|| Some(utoipa::openapi::path::ParameterIn::Query))
                ...
                .map(|param| {
                    ...
                    ParameterSpec {
                        location: param.parameter_in.into(),
                        field: FieldSpec {
                            name: param.name,
                            ty: param
                                .schema
    ...
    where
        T: ts_rs::TS + 'static,
    {
        self.request_body_with_description::<T>(None);
    }

    pub fn request_body_with_description<T>(&mut self, description: Option<&'static str>)
    where
        T: ts_rs::TS + 'static,
    {
        self.spec.request_body = Some(RequestBodySpec {
            body: self.collector.type_ref::<T>(),
            description: description.map(str::to_owned),
        });
    }
    ...
    where
        T: ts_rs::TS + 'static,
    {
        self.response_with_description::<T>(status, None);
    }

    pub fn response_with_description<T>(
        &mut self,
        status: &'static str,
        description: Option<&'static str>,
    ) where
        T: ts_rs::TS + 'static,
    {
        self.response_with_metadata::<T>(status, description, None);
    }

    pub fn response_with_metadata<T>(
        &mut self,
        status: &'static str,
        description: Option<&'static str>,
    ...
    pub fn empty_response(&mut self, status: &'static str) {
        self.empty_response_with_description(status, None);
    }

    pub fn empty_response_with_description(
        &mut self,
        status: &'static str,
        description: Option<&'static str>,
    ) {
    ...
    pub fn empty_response_with_metadata(
        &mut self,
        status: &'static str,
        description: Option<&'static str>,
        content_type: Option<&'static str>,
...
fn render_file(declarations: &BTreeMap<String, String>, endpoints: &[EndpointSpec]) -> String {
    ...
    let mut paths = BTreeMap::<&str, Vec<&EndpointSpec>>::new();
    for endpoint in endpoints {
        paths.entry(endpoint.path).or_default().push(endpoint);
    }

    for (path, mut endpoints) in paths {
        endpoints.sort_by_key(|endpoint| (endpoint.method, endpoint.name));

        let _ = writeln!(out, "  {}: {{", ts_key(path));

        for endpoint in endpoints {
            let _ = writeln!(out, "    {}: {{", ts_key(endpoint.method));

            if !endpoint.params.is_empty() {
                write_parameters_object(&mut out, &endpoint.params, 6);
            }

            if let Some(body) = &endpoint.request_body {
                write_description(&mut out, body.description.as_deref(), 6);
                let _ = writeln!(out, "      body: {};", body.body);
            }

            out.push_str("      responses: {\n");
            for response in &endpoint.responses {
                write_description(&mut out, response.description.as_deref(), 8);
                let body = response.body.as_deref().unwrap_or("never");
                let _ = writeln!(out, "        {}: {{", ts_key(response.status));
                let _ = writeln!(out, "          body: {};", body);
                if let Some(content_type) = &response.content_type {
                    let _ = writeln!(out, "          contentType: {};", ts_string(content_type));
                }
                if !response.headers.is_empty() {
                    let fields = response.headers.iter().collect::<Vec<_>>();
                    write_fields_object(&mut out, "headers", "", &fields, 10);
                }
                out.push_str("        };\n");
            }
            out.push_str("      };\n");
...
fn write_parameters_object(out: &mut String, params: &[ParameterSpec], indent: usize) {
    ...
    let _ = writeln!(out, "{padding}parameters: {{");
    ...
    for location in LOCATIONS {
        ...
        let optional = if fields.iter().any(|field| field.required) {
            ""
        } else {
            "?"
        };
        write_fields_object(out, location.name(), optional, &fields, indent + 2);
    }

    let _ = writeln!(out, "{padding}}};");
}

fn write_fields_object(
    out: &mut String,
    name: &str,
    optional: &str,
    fields: &[&FieldSpec],
    ...
    let padding = " ".repeat(indent);
    if fields.is_empty() {
        let _ = writeln!(out, "{padding}{name}{optional}: Record<string, never>;");
        return;
    }

    let _ = writeln!(out, "{padding}{name}{optional}: {{");
    for field in fields {
        let optional = if field.required { "" } else { "?" };
        write_description(out, field.description.as_deref(), indent + 2);
        let _ = writeln!(
            out,
            "{padding}  {}{}: {};",
            ts_key(&field.name),
...
fn write_description(out: &mut String, description: Option<&str>, indent: usize) {
    ...
    if !description.contains('\n') {
        let _ = writeln!(out, "{padding}/** @description {description} */");
        return;
    }
    ...
    for (index, line) in description.lines().enumerate() {
        if index == 0 {
            let _ = writeln!(out, "{padding} * @description {line}");
        } else {
...
fn schema_ref_description(
    schema: &utoipa::openapi::RefOr<utoipa::openapi::schema::Schema>,
) -> Option<String> {
    match schema {
        utoipa::openapi::RefOr::T(schema) => schema_description(schema),
        utoipa::openapi::RefOr::Ref(reference) => {
            (!reference.description.is_empty()).then(|| reference.description.clone())
        }
    }
}

fn schema_description(schema: &utoipa::openapi::schema::Schema) -> Option<String> {
    match schema {
        utoipa::openapi::schema::Schema::Object(object) => object.description.clone(),
        utoipa::openapi::schema::Schema::Array(array) => array.description.clone(),
        utoipa::openapi::schema::Schema::OneOf(one_of) => one_of.description.clone(),
        utoipa::openapi::schema::Schema::AllOf(all_of) => all_of.description.clone(),
        utoipa::openapi::schema::Schema::AnyOf(any_of) => any_of.description.clone(),
        _ => None,
    }
}
    ...
    if !object.properties.is_empty() {
        let fields = object
            ...
            .map(|(name, schema)| FieldSpec {
                ...
                description: schema_ref_description(schema),
            })
            .collect::<Vec<_>>();
        let mut out = String::new();
        out.push_str("{\n");
        ...
        for field in fields {
            let optional = if field.required { "" } else { "?" };
            write_description(&mut out, field.description.as_deref(), 2);
